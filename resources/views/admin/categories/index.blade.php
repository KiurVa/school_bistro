...
@section('content')
    <div class="container mt-4">
        ...
        @forelse($menuTypes as $menuType)
            @php
                $typeCategories = $categories->where('menu_type_id', $menuType->id)->sortBy('order_index')->values();
            @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <form action="{{ route('categories.move_up', $category) }}"
                                                    ...
                                                    <form action="{{ route('categories.move_down', $category) }}"
                                                        method="POST" class="d-inline">
                                                        @csrf
                                                        <button type="submit" class="btn btn-outline-secondary"
                                                            {{ $loop->last ? 'disabled' : '' }}>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <form action="{{ route('categories.destroy', $category) }}" method="POST"
                                                class="d-inline" onsubmit="return confirm('Kustutada kategooria?');">
                                                @csrf
                                                @method('DELETE')
